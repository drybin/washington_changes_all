...
func generateReport(day model.DayResult) string {
    ...
    strategyText := fmt.Sprintf(
        "монета упала на <b>%.2f%%</b> (<i>от средней цены %.8f</i>) текущая цена <b>%.8f</b>"+newLine,
        day.CoinPriceChange,
    ...
    if day.AmountEmptyStrategy {
        ...
        strategyText = fmt.Sprintf(
            "монета упала на <b>%.2f%%</b> (<i>от ath %.8f</i>) текущая цена <b>%.8f</b>"+newLine,
            day.CoinPriceChange,
            day.CoinAth,
            day.Price,
        )
    ...
    return fmt.Sprintf("Сегодня <b>%d</b> день (<i>дата %d-%02d-%02d</i>)"+newLine+
        ...
        "%s"+
        "%s"+
        "Купили <b>%.8f</b>"+newLine+
        "Новая средняя цена монеты %.8f (<i>предыдущая %.8f</i>)"+newLine+
