...
func (s *DayProcessSellService) Process(
    ...
    marketsHistory, err := s.CryptoExchangeService.GetMarketsHistory(ctx)
    if err != nil {
    ...
    balance, err := s.CryptoExchangeService.GetBalance(ctx)
    if err != nil {
...
func (s *DayProcessSellService) sell(
    ...
    //byu on kukoin
    orderInfo, err := s.CryptoExchangeService.SellByMarket(
        ctx,
        model.Coin{
            Name: coin,
        },
